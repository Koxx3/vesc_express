...
static void char1_write_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
	...
	buffer_display("char1_write_handler : ", param->write.value, param->write.len);
	...
	{
		printf("char1_write_handler : need_rsp\n");
...
{
	printf("process_packet : len = %d / %s\n", len, getDataTypeStr(data[0]));
	buffer_display("process_packet : ", data, len);
...
static void send_packet_raw(unsigned char *buffer, unsigned int len)
{
	...
	printf("send_packet_raw : len = %d / %s\n", len, getDataTypeStr(cmd));
	buffer_display("send_packet_raw : ", buffer, len);
	...
	while (bytes_sent < len)
	{
		...
		printf("send_packet_raw : packet = %d / length = %d\n", iPacket, length);
		buffer_display("send_packet_raw PART : ", buffer + bytes_sent, length);
...
void comm_ble_init(void)
{
	...
	esp_ble_tx_power_set(ESP_BLE_PWR_TYPE_ADV, ESP_PWR_LVL_P18);
	esp_ble_tx_power_set(ESP_BLE_PWR_TYPE_SCAN, ESP_PWR_LVL_P18);
	esp_ble_tx_power_set(ESP_BLE_PWR_TYPE_DEFAULT, ESP_PWR_LVL_P18); // ESP_PWR_LVL_P18
...
{
	// buffer_display("comm_ble_send_packet : ", data, len);
	printf("comm_ble_send_packet : len = %d / %s\n", len, getDataTypeStr(data[0]));
